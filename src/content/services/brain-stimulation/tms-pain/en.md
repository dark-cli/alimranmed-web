---
title: "TMS for pains"
description: "Repetitive transcranial magnetic stimulation (rTMS) is a non-invasive treatment for neuropathic pain, including central pain, fibromyalgia and migraine."
category: "brain-stimulation"
order: 999
legacyUrl: "https://alimranmed.com/repetitive-transcranial-magnetic-stimulation-rtms-for-treating-various-pain-conditions/"
source: "legacy-wp"
redesigned: true
sections:
  - type: at_a_glance
    items:
      - label: "What it is"
        value: "rTMS applies repetitive magnetic pulses to the brain to disrupt aberrant pain signalling pathways without drugs or surgery."
      - label: "What it treats"
        value: "Neuropathic pain, central pain, peripheral nerve disorders, fibromyalgia, and migraine. Emerging evidence supports use in orofacial pain, trigeminal neuralgia, phantom pain, low back pain, and CRPS."
      - label: "What to expect"
        value: "Outpatient sessions with no anaesthesia required. Treatment is delivered as a course, with effects building progressively."
  - type: prose
    heading: "TMS for pain at Alimran Medical Center"
    body: |
      Repetitive transcranial magnetic stimulation (rTMS) has established evidence for treating neuropathic pain originating from central or peripheral nervous system dysfunction, including fibromyalgia and migraine. By modulating cortical excitability and descending pain control pathways, rTMS can reduce the intensity and frequency of chronic pain without pharmacological side effects.

      Evidence is strong for central pain, peripheral nerve disorders, fibromyalgia, and migraine. Promising but still-developing evidence supports rTMS for orofacial pain, trigeminal neuralgia, phantom limb pain, low back pain, myofascial pain, pelvic pain, and complex regional pain syndrome (CRPS).

      Treatment is conducted on an outpatient basis with no anaesthesia. Sessions are brief and patients return to daily activities immediately. A course of treatment is typically required for sustained benefit, and protocols are tailored to the specific pain condition.
  - type: treatment_groups
    heading: "Treatment options at Alimran Medical Center"
    groups:
      - title: "Brain stimulation"
        items:
          - "[TMS](/services/brain-stimulation/tms/)"
          - "[tDCS](/services/brain-stimulation/tdcs/)"
          - "[TMS for migraine](/services/brain-stimulation/tms-migraine/)"
      - title: "Radiofrequency"
        items:
          - "[Radiofrequency](/services/radiofrequency/radiofrequency/)"
          - "[Spine radiofrequency](/services/radiofrequency/spine/)"
      - title: "Injection therapies"
        items:
          - "[Ozone therapy](/services/ozone-therapy/review/)"
          - "[Prolotherapy](/services/regenerative-medicine/prolotherapy/)"
---
