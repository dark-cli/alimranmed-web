---
title: "Multiple sclerosis (MS)"
description: "A chronic autoimmune disease in which the immune system attacks the myelin sheath of nerve fibres, disrupting nerve conduction in the brain and spinal cord. Produces variable neurological symptoms. Managed with medication, rTMS, physiotherapy, and rehabilitation."
category: "pain"
order: 999
legacyUrl: "https://alimranmed.com/2020/04/09/multiple-sclerosis/"
publishedAt: "2020-04-09T10:55:27"
source: "legacy-wp"
redesigned: true
sections:
  - type: at-a-glance
    items:
      - label: "What it is"
        value: "A chronic autoimmune disease of the central nervous system in which the immune system attacks the myelin sheath surrounding nerve fibres, impairing signal transmission."
      - label: "Symptoms"
        value: "Loss of strength, fatigue, visual disturbances, sensory changes, balance and coordination problems, sphincter dysfunction, cognitive and emotional difficulties. Symptoms depend on lesion location."
      - label: "Approach"
        value: "Interdisciplinary care: medication to modify disease course, rTMS and spinal cord stimulation for symptoms, physiotherapy and rehabilitation to restore function and independence."
  - type: prose
    heading: "What multiple sclerosis is"
    body: |
      Multiple sclerosis is a chronic disease of the central nervous system in which the immune system attacks the myelin sheath surrounding nerve fibres. Myelin is responsible for efficient transmission of nerve impulses in the brain and spinal cord. Where myelin is destroyed, hardened plaques (sclerosis) form and nerve conduction is impaired.

      The disease initially presents in episodic attacks with exacerbations and remissions, but often evolves into a progressive form or produces stable neurological deficits that accumulate over time.

      **Clinical manifestations** depend on the location and extent of lesions across the spinal cord, brain, and cerebellum:
      - Loss of muscle strength and coordination
      - Fatigue
      - Visual disturbances
      - Sensory disturbances (tingling, numbness)
      - Bladder and bowel dysfunction
      - Sexual dysfunction
      - Cognitive and emotional difficulties

      Although pharmacological treatments can modify the course of the disease, diagnosing and treating complications requires an interdisciplinary team of specialists working with a unified care plan tailored to each patient's deficits.

      **Rehabilitation goals:** Restore the affected part to function and reduce dependence on others. Includes physical, adaptive, cognitive, social, functional, and speech rehabilitation.
  - type: treatment-groups
    heading: "Treatment options at Alimran Medical Center"
    groups:
      - title: "Neuromodulation"
        items:
          - "[Repetitive transcranial magnetic stimulation (rTMS)](/services/brain-stimulation/tms/)"
          - "[Spinal cord stimulation](/services/radiofrequency/epidural-adhesiolysis/)"
          - "[Intrathecal baclofen pump](/services/surgery/intrathecal-pump/)"
      - title: "Injection therapies"
        items:
          - "Trigger point injections"
          - "Epidural steroid injections"
          - "[Botox® injections](/services/botox/botox-injection/)"
          - "[Ozone injection](/services/ozone-therapy/review/)"
          - "[Prolotherapy](/services/regenerative-medicine/prolotherapy/)"
      - title: "Advanced therapies"
        items:
          - "[Pulsed radiofrequency](/services/radiofrequency/radiofrequency/)"
          - "[Acupuncture](/services/acupuncture/acupuncture/)"
      - title: "Physiotherapy & rehabilitation"
        items:
          - "[Electrical stimulation](/services/physiotherapy/electrical-stimulation/)"
          - "[Short and long wave therapy](/services/physiotherapy/shortwave-therapy/)"
          - "[Laser therapy](/services/physiotherapy/laser-therapy/)"
          - "[Ultrasound therapy](/services/physiotherapy/ultrasound-therapy/)"
          - "Therapeutic and strengthening exercises"
          - "[SpineMED® system](/services/chiropractic/spinmed/)"
          - "[Sigma](/services/chiropractic/sigma/)"
---
