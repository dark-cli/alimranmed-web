---
title: "Subdural hematoma"
description: "A blood clot that forms beneath the skull and the dura mater, outside the brain — usually caused by head injury. Ranges from mild (conservatively managed) to severe (requiring surgical drainage). Post-acute rehabilitation is key to recovery."
category: "brain"
order: 999
legacyUrl: "https://alimranmed.com/subdural-hematoma/"
source: "legacy-wp"
redesigned: true
sections:
  - type: at_a_glance
    items:
      - label: "What it is"
        value: "A blood clot (hematoma) that forms between the skull and the outer membrane covering the brain (dura mater), typically following a head injury."
      - label: "Symptoms"
        value: "Headache, confusion, loss of consciousness, blurred vision, vomiting, weakness on one side of the body, and slurred speech. Requires immediate medical attention."
      - label: "Approach"
        value: "Severity determines treatment. Mild cases are managed conservatively. Severe cases may require surgical drainage (burr hole). Post-acute rehabilitation addresses residual deficits."
  - type: prose
    heading: "What a subdural hematoma is"
    body: |
      A subdural hematoma is a type of intracranial hematoma — a collection of blood that forms beneath the skull and beneath the dura mater (the tough membrane surrounding the brain) but outside the brain tissue itself. It typically results from a tear in the veins passing between the brain and the dura, or from a laceration on the brain itself. A skull fracture may or may not be present.

      **Symptoms** vary with injury severity. Moderate to severe head injury requires immediate medical attention:
      - Severe headache
      - Confusion or disorientation
      - Loss of consciousness
      - Blurred vision
      - Vomiting
      - Short-term memory loss
      - Slurred speech, difficulty walking, or dizziness
      - Weakness or numbness on one side of the body
      - Seizures or behaviour changes
      - One pupil appearing larger than the other

      **Raised intracranial pressure:** Head injury may cause brain swelling. Because the skull has limited space, swelling raises pressure inside the skull (intracranial pressure, ICP) — which can lead to brain damage. Severe cases require ICP monitoring.

      **Surgical option:** When the hematoma is significant and does not resolve, burr hole evacuation (surgical drainage) may be required.
  - type: treatment_groups
    heading: "Non-surgical treatment at Alimran Medical Center"
    groups:
      - title: "Neuromodulation"
        items:
          - "[Repetitive transcranial magnetic stimulation (rTMS)](/services/brain-stimulation/tms/)"
          - "[Botox® injections](/services/botox/botox-injection/)"
      - title: "Advanced therapies"
        items:
          - "[Pulsed radiofrequency](/services/radiofrequency/radiofrequency/)"
          - "[Acupuncture](/services/acupuncture/acupuncture/)"
      - title: "Physiotherapy & rehabilitation"
        items:
          - "[Electrical stimulation](/services/physiotherapy/electrical-stimulation/)"
          - "[Short and long wave therapy](/services/physiotherapy/shortwave-therapy/)"
          - "[Laser therapy](/services/physiotherapy/laser-therapy/)"
          - "[Ultrasound therapy](/services/physiotherapy/ultrasound-therapy/)"
          - "Therapeutic exercises"
          - "[Sigma](/services/chiropractic/sigma/)"
---
