---
locale: "ar"
title: "ألم الفرج المزمن (Vulvodynia)"
description: "ألم مزمن أو إزعاج في منطقة الفرج لا سبب محدد له ويستمر ثلاثة أشهر على الأقل. قد يُسبّب حرقاناً ووخزاً وألماً عند الجماع. يُعالَج بمزيج من الحقن والتعديل العصبي والعلاج الطبيعي."
category: "الألم"
order: 999
legacyUrl: "https://ar.alimranmed.com/التهاب-الأعضاء-التناسلية-الأنثوية"
source: "legacy-wp"
redesigned: true
sections:
  - type: at_a_glance
    items:
      - label: "ما هو"
        value: "ألم مزمن أو إزعاج في منطقة الفرج لا يمتد لأقل من ثلاثة أشهر ولا سبب عضوي محدد له."
      - label: "الأعراض"
        value: "حرقان، وخز، تقرّح، تسلّخ، ألم عند الجماع، نبض مؤلم، أو حكة. قد يكون مستمراً أو يُحفَّز باللمس؛ عاماً أو موضعياً."
      - label: "المقاربة"
        value: "لا علاج واحد يناسب الجميع. يُصمَّم مزيج من الحقن والـ rTMS والعلاج الطبيعي لكل حالة. قد يحتاج التحسّن بعض الوقت."
  - type: prose
    heading: "ما هو ألم الفرج المزمن"
    body: |
      ألم الفرج المزمن هو ألم أو إزعاج مستمر في منطقة الفرج دون سبب عضوي محدد، يدوم ثلاثة أشهر على الأقل. قد يُصبح الجلوس لفترات طويلة أو ممارسة الجنس أمراً شبه مستحيل. وتستمر الحالة أحياناً لشهور أو سنوات.

      قد يكون الألم مستمراً أو متقطعاً، يظهر عند اللمس (مُستثار) أو تلقائياً. قد يشمل منطقة الفرج كلها (عام) أو يقتصر على منطقة بعينها كفتحة المهبل (الدهليز). وكثيراً ما تبدو أنسجة منطقة الفرج طبيعية تماماً.

      حالة مشابهة تُدعى التهاب دهليز الفرج، تُسبّب الألم فقط عند الضغط على المنطقة المحيطة بمدخل المهبل.

      **العوامل المحتملة المساهمة في الحالة:**
      - إصابة الأعصاب المحيطة بمنطقة الفرج أو تهيجها
      - عدوى مهبلية سابقة
      - حساسية الجلد أو التحسس من مواد معينة
      - تغيرات هرمونية
      - تشنج أو ضعف في عضلات قاع الحوض

      إذا كنتِ تعانين من هذه الحالة، فلا تسمحي للعلامات غير الظاهرة أو الحرج من مناقشتها أن يمنعك من طلب المساعدة — فالعلاجات الفعّالة متاحة.
  - type: treatment_groups
    heading: "خيارات العلاج في مركز العمران"
    groups:
      - title: "علاجات الحقن"
        items:
          - "حقن نقاط الزناد"
          - "حقن الستيرويد فوق الجافية"
          - "[حقن البوتوكس®](/services/botox/botox-injection/)"
          - "[برولوثيرابي](/services/regenerative-medicine/prolotherapy/)"
      - title: "التعديل العصبي"
        items:
          - "[التحفيز المغناطيسي المتكرر عبر الجمجمة (rTMS)](/services/brain-stimulation/tms/)"
          - "[التحفيز الكهربائي المباشر عبر الجمجمة (tDCS)](/services/brain-stimulation/tdcs/)"
          - "[تحفيز النخاع الشوكي](/services/radiofrequency/epidural-adhesiolysis/)"
          - "[المضخة العلاجية داخل القناة الشوكية](/services/surgery/intrathecal-pump/)"
      - title: "العلاجات المتقدمة"
        items:
          - "[الترددات الراديوية النبضية](/services/radiofrequency/radiofrequency/)"
          - "[الوخز بالإبر](/services/acupuncture/acupuncture/)"
          - "[سيغما](/services/chiropractic/sigma/)"
      - title: "العلاج الطبيعي"
        items:
          - "[التحفيز الكهربائي](/services/physiotherapy/electrical-stimulation/)"
          - "[العلاج بالموجات القصيرة والطويلة](/services/physiotherapy/shortwave-therapy/)"
          - "[العلاج بالليزر](/services/physiotherapy/laser-therapy/)"
          - "[العلاج بالموجات فوق الصوتية](/services/physiotherapy/ultrasound-therapy/)"
          - "العلاج بالموجات المغناطيسية"
          - "التمارين العلاجية"
---
