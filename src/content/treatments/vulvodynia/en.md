---
title: "Vulvodynia"
description: "Chronic pain or discomfort around the vulva with no identifiable cause, lasting at least three months. May cause burning, stinging, and painful intercourse. Treated with a combination of injections, neuromodulation, and physiotherapy."
category: "pain"
order: 999
legacyUrl: "https://alimranmed.com/vulvodynia/"
source: "legacy-wp"
redesigned: true
sections:
  - type: at_a_glance
    items:
      - label: "What it is"
        value: "Chronic pain or discomfort in the vulvar area lasting at least three months, with no identifiable underlying cause."
      - label: "Symptoms"
        value: "Burning, stinging, soreness, rawness, painful intercourse, throbbing, or itching. Pain may be constant or triggered by touch; generalised or localised."
      - label: "Approach"
        value: "No single treatment works for all patients. A combination of injections, rTMS, and physiotherapy is tailored to each case. Relief may take time."
  - type: prose
    heading: "What vulvodynia is"
    body: |
      Vulvodynia is chronic pain or discomfort around the opening of the vagina (vulva) for which there is no identifiable cause, lasting at least three months. The pain, burning, or irritation can make sitting for long periods or having sex extremely uncomfortable. The condition can persist for months to years.

      The pain may be constant or intermittent, triggered by touch (provoked), or spontaneous. It may affect the entire vulvar area (generalised) or a specific region such as the vaginal opening (vestibule). Vulvar tissue often looks completely normal.

      A related condition, vestibulodynia, causes pain only when pressure is applied to the area surrounding the vaginal entrance.

      **Possible contributing factors:**
      - Injury to or irritation of the nerves surrounding the vulvar region
      - Previous vaginal infections
      - Allergies or skin sensitivity
      - Hormonal changes
      - Pelvic floor muscle spasm or weakness

      If you have vulvodynia, the absence of visible signs and embarrassment should not prevent you from seeking help — effective treatments are available.
  - type: treatment_groups
    heading: "Treatment options at Alimran Medical Center"
    groups:
      - title: "Injection therapies"
        items:
          - "Trigger point injections"
          - "Epidural steroid injections"
          - "[Botox® injections](/services/botox/botox-injection/)"
          - "[Prolotherapy](/services/regenerative-medicine/prolotherapy/)"
      - title: "Neuromodulation"
        items:
          - "[Repetitive transcranial magnetic stimulation (rTMS)](/services/brain-stimulation/tms/)"
          - "[Transcranial direct current stimulation (tDCS)](/services/brain-stimulation/tdcs/)"
          - "[Spinal cord stimulation](/services/radiofrequency/epidural-adhesiolysis/)"
          - "[Intrathecal pump](/services/surgery/intrathecal-pump/)"
      - title: "Advanced therapies"
        items:
          - "[Pulsed radiofrequency](/services/radiofrequency/radiofrequency/)"
          - "[Acupuncture](/services/acupuncture/acupuncture/)"
          - "[Sigma](/services/chiropractic/sigma/)"
      - title: "Physiotherapy"
        items:
          - "[Electrical stimulation](/services/physiotherapy/electrical-stimulation/)"
          - "[Short and long wave therapy](/services/physiotherapy/shortwave-therapy/)"
          - "[Laser therapy](/services/physiotherapy/laser-therapy/)"
          - "[Ultrasound therapy](/services/physiotherapy/ultrasound-therapy/)"
          - "Magnetic therapy"
          - "Therapeutic exercises"
---
